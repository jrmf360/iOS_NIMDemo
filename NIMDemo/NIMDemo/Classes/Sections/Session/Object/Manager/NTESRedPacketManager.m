...
- (void)onLogin:(NIMLoginStep)step
{
    switch (step)
    {
        case NIMLoginStepSyncOK:
        {
            ...
            BOOL isOnLine = [JRMFSington GetPacketSington].isOnline;
            NSString *aliPaySchemeUrl = [JRMFSington GetPacketSington].aliPaySchemeUrl;
            NSString *weChatSchemeUrl = [JRMFSington GetPacketSington].weChatSchemeUrl;
            [[JRMFRedPacketManager sharedManager] fetchTokenWithRedPacketRequest:request completion:^(NSError * _Nullable error, NSString * _Nullable token) {
                if (!error)
                {
